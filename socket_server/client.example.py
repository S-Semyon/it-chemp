...
def split_send_bytes(s, inp):
    ...
    if data_len == 0:
        print(
            "ERROR: trying to send 0 bytes"
        )  # should not happen in theory but threads are weird
        return
...
class SharedBuf:
    ...
    def extbuf(self, arr):
        ...
        if arr_len + self.write_cursor > self.size:
            ...
            self.buffer[: arr_len - (self.size - self.write_cursor)] = arr[
    ...
    def getx(self, x):
        ...
        if self.read_cursor + x > self.size:
            ret = np.append(
                self.buffer[self.read_cursor :],
                self.buffer[: self.read_cursor + x - self.size],
            )
        else:
        ...
        while running:
            ...
            if data is not None:
                with item_available:
                    ...
                    if item_available.wait_for(
                        lambda: buf.getlen() <= SHARED_BUF_SIZE, timeout=2
                    ):
                        buf.extbuf(data)
    ...
    def transmitter_consumer(buf, serversocket):
        ...
        while running:
            ...
            with item_available:
                ...
                item_available.wait_for(
                    lambda: buf.getlen() >= TX_BATCH_SIZE, timeout=2
                )
        ...
        while running:
            ...
            with audio_available:
                if buff.getlen() < PLAYER_READ_BYTE_SIZE:
                    # if buffer is empty, wait for it to be filled
                    audio_available.wait_for(
                        lambda: buff.getlen() >= PLAYER_READ_LAG_SIZE, timeout=2
                    )
...
def connect():
    ...
    print(
        f"message length = {len((source_name + (' ' * (512 - len(source_name)))).encode())}"
    )
